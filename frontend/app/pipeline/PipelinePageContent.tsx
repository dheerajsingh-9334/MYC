...
export default function PipelinePage() {
  ...
  const { data: liveStats, isLoading: loadingStats }   = useQuery({ queryKey: ['dashboard-stats'], queryFn: () => apiFetch('/api/dashboard/stats'),  enabled: !USE_MOCK, retry: false });
  const { data: liveSteps = [], isLoading: loadingSteps }  = useQuery({ queryKey: ['steps'],          queryFn: () => apiFetch('/api/steps'),            enabled: !USE_MOCK, retry: false });
  const { data: liveClients = [], isLoading: loadingClients }= useQuery({ queryKey: ['clients'],        queryFn: () => apiFetch('/api/clients'),          enabled: !USE_MOCK, retry: false });
  const { data: liveTasks = [], isLoading: loadingTasks }  = useQuery({ queryKey: ['tasks'],          queryFn: () => apiFetch('/api/tasks'),            enabled: !USE_MOCK, retry: false });
  const { data: liveStandup, isLoading: loadingStandup }     = useQuery({ queryKey: ['standup'],        queryFn: () => apiFetch('/api/standup'),          enabled: !USE_MOCK, retry: false });

  const isLoading = !USE_MOCK && (loadingStats || loadingSteps || loadingClients || loadingTasks || loadingStandup);
