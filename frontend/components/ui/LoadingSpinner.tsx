...
/**
 * BtnSpinner — tiny inline spinner for inside action buttons.
 * Usage:  {mut.isPending ? <BtnSpinner /> : null}  alongside button label
 */
export function BtnSpinner({ size = 13, color = 'currentColor' }: { size?: number; color?: string }) {
  return (
    <>
      <SpinnerStyle />
      <svg
        className="animate-spinner-rotate"
        style={{ width: size, height: size, color, display: 'inline-block', verticalAlign: 'middle', flexShrink: 0 }}
        xmlns="http://www.w3.org/2000/svg"
        fill="none"
        viewBox="0 0 24 24"
      >
